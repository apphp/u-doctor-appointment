...
class News extends CActiveRecord
{
		...
		if($keywords !== ''){
			...
			foreach($news as $key => $val){
				$result[0][] = array(
					...
					'intro_image'	=> (!empty($val['intro_image']) ? '<img class="search-image" src="assets/modules/news/images/intro_images/'.CHtml::encode($val['intro_image']).'" alt="news intro" />' : ''),
	...
	protected function _clearCache()
	{
		if(CConfig::get('cache.enable')){
			...
			$cacheFile = CConfig::get('cache.path').md5('news-findall-2').'.cch';
