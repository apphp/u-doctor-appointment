...
(NULL, 'news', 'News', 'News', 'News module allows creating and displaying news on the site', '0.0.4', 'icon.png', 0, 0, 1, 0, 1, '<CURRENT_DATETIME>', NULL, 0, (SELECT COUNT(m.id) + 1 FROM `<DB_PREFIX>modules` m WHERE m.is_system = 0));
...
INSERT INTO `<DB_PREFIX>frontend_menus` (`id`, `parent_id`, `menu_type`, `module_code`, `link_url`, `link_target`, `placement`, `sort_order`, `access_level`, `is_active`) VALUES (NULL, 0, 'pagelink', 'news', 'news/viewAll', '', 'top', 3, 'public', 1);
...
INSERT INTO `<DB_PREFIX>frontend_menus` (`id`, `parent_id`, `menu_type`, `module_code`, `link_url`, `link_target`, `placement`, `sort_order`, `access_level`, `is_active`) VALUES (NULL, 0, 'moduleblock', 'news', 'drawNewsBlock', '', 'right', 1, 'public', 1);
INSERT INTO `<DB_PREFIX>frontend_menu_translations` (`id`, `menu_id`, `language_code`, `name`) SELECT NULL, (SELECT MAX(id) FROM `<DB_PREFIX>frontend_menus`), code, 'News' FROM `<DB_PREFIX>languages`;
...
(NULL, 'news', 'news', 'News', 'News', 'search', '20', (SELECT COUNT(sc.id) + 1 FROM `<DB_PREFIX>search_categories` sc), 1);
...
INSERT INTO `<DB_PREFIX>news` (`id`, `intro_image`, `created_at`, `is_published`) VALUES (1, '1.png', '2018-02-01 12:45:00', 1);
INSERT INTO `<DB_PREFIX>news` (`id`, `intro_image`, `created_at`, `is_published`) VALUES (2, '2.png', '2018-02-21 11:12:30', 1);
...
INSERT INTO `<DB_PREFIX>news_translations` (`id`, `news_id`, `language_code`, `news_header`, `news_text`) SELECT NULL, 1, code, 'Center for Medical Technology Innovation', ' Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean gravida, enim nec scelerisque tristique, quam neque pharetra mauris, nec bibendum massa metus finibus neque. Duis sed cursus sem, sed vestibulum tortor. Sed suscipit tellus nibh, eu ultrices velit maximus vel. Etiam fringilla blandit tortor id commodo. Cras elementum nisi at tempus malesuada. Duis commodo elementum erat, at interdum ipsum ultrices at. Morbi id rhoncus arcu, ac porttitor magna. Sed feugiat ipsum ac sem tempus sodales. Nullam non lectus mi. Nulla orci elit, scelerisque eu nisi eu, fermentum sodales quam. Vivamus tempus velit eu odio posuere blandit. Nullam sit amet semper leo. Sed aliquam pharetra quam, id efficitur lectus vulputate quis. Nam blandit est risus, vitae luctus purus scelerisque eu. Cras ullamcorper quis massa eget placerat. Quisque risus risus, rhoncus vel orci ut, ullamcorper viverra mauris. Donec rhoncus orci et pellentesque bibendum. In tempor congue congue. ' FROM `<DB_PREFIX>languages`;
INSERT INTO `<DB_PREFIX>news_translations` (`id`, `news_id`, `language_code`, `news_header`, `news_text`) SELECT NULL, 2, code, 'Nutritional Personal Consultation', ' Mauris eget sagittis felis. Praesent convallis lorem et tristique imperdiet. Pellentesque in neque purus. Fusce justo nulla, euismod nec enim vel, bibendum egestas magna. Pellentesque porta scelerisque ex in cursus. Duis dictum nibh a justo ornare imperdiet. Sed iaculis pellentesque erat, sed bibendum nunc vestibulum vel. Integer semper hendrerit metus congue euismod. Morbi fermentum suscipit ligula eget molestie. Mauris cursus lorem tellus. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Integer sollicitudin ultricies ornare. Vivamus ac enim accumsan, sodales metus sed, tristique quam. Nulla a erat et est imperdiet ornare. In id cursus nibh. Donec sed posuere lacus. Duis imperdiet, risus at dictum suscipit, nisi tellus cursus metus, posuere mollis sapien justo eu tellus. Cras vulputate sit amet quam in vulputate. Aliquam sed interdum lectus. Donec dignissim lorem id lobortis faucibus. Aliquam efficitur eros ac est hendrerit dignissim. Suspendisse eget fermentum mauris. Vestibulum sollicitudin sapien vitae orci placerat, ut pellentesque nulla scelerisque. Nunc rhoncus rutrum feugiat. Duis in congue augue, sit amet rutrum diam. Proin venenatis quam nec nunc laoreet sagittis. ' FROM `<DB_PREFIX>languages`;
