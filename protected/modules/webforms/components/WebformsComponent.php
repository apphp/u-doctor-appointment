<?php
/**
 * Webforms component for building web-forms dynamically
 *
 * PUBLIC (static):         PRIVATE:
 * -----------              ------------------
 * drawShortcode
 *
 */

class WebformsComponent extends CComponent
{
	
	const NL = "\n";
	
	/**
	 *	Returns the instance of object
	 *	@return current class
	 */
	public static function init()
	{
		return parent::init(__CLASS__);
	}

	/**
	 * Draws web form
	 */
	public static function drawShortcode()
	{
		$output = '';
		$fieldName = '';
		$fieldEmail = '';
		$fieldPhone = '';
		$fieldMessage = '';
		$fieldCompany = '';
		$fieldCaptcha = '';

		$output .= CHtml::openTag('div', array('class'=>'entry'));
		$output .= CHtml::openTag('div', array('id'=>'cmsms_cc'));

		$output .= CHtml::openTag('div', array('id'=>'cmsms-form-builder'));
		$output .= CHtml::openForm('WebForms/submit', 'post', array('name'=>'form-contact')).self::NL;

		$output .= CHtml::openTag('div', array('class'=>'first_column margin-bottom-20 margin-top-20', 'style'=>'display:none', 'id'=>'messageSuccess'));
			$output .= CHtml::openTag('aside', array('class'=>'box success_box'));
				$output .= CHtml::openTag('table', array());
					$output .= CHtml::openTag('tbody', array());
						$output .= CHtml::openTag('tr', array());
							$output .= CHtml::tag('td', array(), "&nbsp;".self::NL);
							$output .= CHtml::openTag('td', array());
								$output .= CHtml::tag('p', array(), A::t('webforms', 'Thanks for your message! We will get back to you ASAP!'));
							$output .= CHtml::closeTag('td').self::NL;
						$output .= CHtml::closeTag('tr').self::NL;
					$output .= CHtml::closeTag('tbody').self::NL;
			 $output .= CHtml::closeTag('table').self::NL;
			$output .= CHtml::closeTag('aside').self::NL;
		$output .= CHtml::closeTag('div').self::NL;

		$output .= CHtml::openTag('div', array('class'=>'contact-form-content'));

		$output .= CHtml::openTag('div', array('class'=>'first_column margin-bottom-20 margin-top-20', 'id'=>'messageInfo'));
			$output .= CHtml::openTag('aside', array('class'=>'box notice_box'));
				$output .= CHtml::openTag('table', array());
					$output .= CHtml::openTag('tbody', array());
						$output .= CHtml::openTag('tr', array());
							$output .= CHtml::tag('td', array(), "&nbsp;".self::NL);
							$output .= CHtml::openTag('td', array());
								$output .= CHtml::tag('p', array(), A::t('webforms', 'Please feel free to contact us')).self::NL;
							$output .= CHtml::closeTag('td').self::NL;
						$output .= CHtml::closeTag('tr').self::NL;
					$output .= CHtml::closeTag('tbody').self::NL;
				$output .= CHtml::closeTag('table').self::NL;
			$output .= CHtml::closeTag('aside').self::NL;
		$output .= CHtml::closeTag('div').self::NL;

		$errorMsg = (APPHP_MODE == 'demo') ? A::t('core', 'This operation is blocked in Demo Mode!') : A::t('webforms', 'An error occurred while sending email! Please try again later.');
		$output .= CHtml::openTag('div', array('class'=>'first_column margin-bottom-20 margin-top-20', 'id'=>'messageError', 'style'=>'display:none'));
			$output .= CHtml::openTag('aside', array('class'=>'box error_box'));
				$output .= CHtml::openTag('table', array());
					$output .= CHtml::openTag('tbody', array());
						$output .= CHtml::openTag('tr', array());
							$output .= CHtml::tag('td', array(), "&nbsp;".self::NL);
							$output .= CHtml::openTag('td', array());
								$output .= CHtml::tag('p', array(), $errorMsg);
							$output .= CHtml::closeTag('td').self::NL;
						$output .= CHtml::closeTag('tr').self::NL;
					$output .= CHtml::closeTag('tbody').self::NL;
				$output .= CHtml::closeTag('table').self::NL;
			$output .= CHtml::closeTag('aside').self::NL;
		$output .= CHtml::closeTag('div').self::NL;

		$fName = ModulesSettings::model()->param('webforms', 'field_name');
		if($fName == 'show-optional' || $fName == 'show-required'){
			$required = ($fName == 'show-required') ? true : false;
			$requiredMark = ($fName == 'show-required') ? ' &#42; ' : '';
			$fieldName .= CHtml::openTag('div', array('class'=>'row form_info cmsms_input'));
				$fieldName .= CHtml::tag('label', array(),A::t('webforms', 'Name').CHtml::tag('span', array('class'=>'color_3'),$requiredMark)).self::NL;
					$fieldName .= CHtml::openTag('div', array('class'=>'form_field_wrap'));
						$fieldName .= CHtml::textField('contact_name', '', array('data-required'=>($required ? 'true' : 'false'), 'maxlength'=>'125', 'autocomplete'=>'off')).self::NL;
					$fieldName .= CHtml::closeTag('div').self::NL; /* form_field_wrap */
				if($required) $fieldName .= CHtml::tag('p', array('class'=>'error', 'style'=>'display:none', 'id'=>'nameError'), A::t('webforms', 'The field name cannot be empty!'));
			$fieldName .= CHtml::closeTag('div').self::NL;
		}

		$fEmail = ModulesSettings::model()->param('webforms', 'field_email');
		if($fEmail == 'show-optional' || $fEmail == 'show-required'){
			$required = ($fEmail == 'show-required') ? true : false;
			$requiredMark = ($fEmail == 'show-required') ? ' &#42; ' : '';
			$fieldEmail .= CHtml::openTag('div', array('class'=>'row form_info cmsms_input'));
				$fieldEmail .= CHtml::tag('label', array(), A::t('webforms', 'Email').CHtml::tag('span', array('class'=>'color_3'),$requiredMark)).self::NL;
					$fieldEmail .= CHtml::openTag('div', array('class'=>'form_field_wrap'));
						$fieldEmail .= CHtml::textField('contact_email', '', array('data-required'=>($required ? 'true' : 'false'), 'maxlength'=>'255', 'autocomplete'=>'off')).self::NL;
					$fieldEmail .= CHtml::closeTag('div').self::NL; /* form_field_wrap */
				if($required){
					$fieldEmail .= CHtml::tag('p', array('class'=>'error', 'style'=>'display:none', 'id'=>'emailErrorEmpty'), A::t('webforms', 'The field email cannot be empty!'));
				}
				$fieldEmail .= CHtml::tag('p', array('class'=>'error', 'style'=>'display:none', 'id'=>'emailErrorValid'), A::t('webforms', 'You must provide a valid email address!'));
			$fieldEmail .= CHtml::closeTag('div').self::NL;
		}

		$fPhone = ModulesSettings::model()->param('webforms', 'field_phone');
		if($fPhone == 'show-optional' || $fPhone == 'show-required'){
			$required = ($fPhone == 'show-required') ? true : false;
			$requiredMark = ($fPhone == 'show-required') ? ' &#42; ' : '';
			$fieldPhone .= CHtml::openTag('div', array('class'=>'row form_info cmsms_input'));
				$fieldPhone .= CHtml::tag('label', array(), A::t('webforms', 'Phone').CHtml::tag('span', array('class'=>'color_3'),$requiredMark)).self::NL;
					$fieldPhone .= CHtml::openTag('div', array('class'=>'form_field_wrap'));
						$fieldPhone .= CHtml::textField('contact_phone', '', array('data-required'=>($required ? 'true' : 'false'), 'maxlength'=>'125', 'autocomplete'=>'off')).self::NL;
					$fieldPhone .= CHtml::closeTag('div').self::NL; /* form_field_wrap */
				if($required) $fieldPhone .= CHtml::tag('p', array('class'=>'error', 'style'=>'display:none', 'id'=>'phoneError'), A::t('webforms', 'The field phone number cannot be empty!'));
			$fieldPhone .= CHtml::closeTag('div').self::NL;
		}

		$fCompany = ModulesSettings::model()->param('webforms', 'field_company');
		if($fCompany == 'show-optional' || $fCompany == 'show-required'){
			$required = ($fCompany == 'show-required') ? true : false;
			$requiredMark = ($fCompany == 'show-required') ? ' &#42; ' : '';
			$fieldCompany .= CHtml::openTag('div', array('class'=>'row form_info cmsms_input'));
				$fieldCompany .= CHtml::tag('label', array(), A::t('webforms', 'Company').CHtml::tag('span', array('class'=>'color_3'),$requiredMark)).self::NL;
					$fieldCompany .= CHtml::openTag('div', array('class'=>'form_field_wrap'));
						$fieldCompany .= CHtml::textField('contact_company', '', array('data-required'=>($required ? 'true' : 'false'), 'maxlength'=>'255', 'autocomplete'=>'off')).self::NL;
					$fieldCompany .= CHtml::closeTag('div').self::NL; /* form_field_wrap */
				if($required) $fieldCompany .= CHtml::tag('p', array('class'=>'error', 'style'=>'display:none', 'id'=>'companyError'), A::t('webforms', 'The field company name cannot be empty!'));
			$fieldCompany .= CHtml::closeTag('div').self::NL;
		}

		$fMessage = ModulesSettings::model()->param('webforms', 'field_message');
		if($fMessage == 'show-optional' || $fMessage == 'show-required'){
			$required = ($fMessage == 'show-required') ? true : false;
			$requiredMark = ($fMessage == 'show-required') ? ' &#42; ' : '';
			$fieldMessage .= CHtml::openTag('div', array('class'=>'row form_info cmsms_input'));
				$fieldMessage .= CHtml::tag('label', array(), A::t('webforms', 'Message').CHtml::tag('span', array('class'=>'color_3'),$requiredMark)).self::NL;
					$fieldMessage .= CHtml::openTag('div', array('class'=>'form_field_wrap'));
						$fieldMessage .= CHtml::textArea('contact_message', '', array('data-required'=>($required ? 'true' : 'false'), 'maxength'=>'2048')).self::NL;
					$fieldMessage .= CHtml::closeTag('div').self::NL; /* form_field_wrap */
				if($required) $fieldMessage .= CHtml::tag('p', array('class'=>'error', 'style'=>'display:none', 'id'=>'messageErrorEmpty'), A::t('webforms', 'The field message cannot be empty!'));
				$fieldMessage .= CHtml::tag('p', array('class'=>'error', 'style'=>'display:none', 'id'=>'messageErrorValid'), A::t('webforms', 'Message must be at least 10 characters in length!'));
			$fieldMessage .= CHtml::closeTag('div').self::NL;
		}

		$fCaptcha = ModulesSettings::model()->param('webforms', 'field_captcha');
		if($fCaptcha == 'show'){
			$fieldCaptcha .= CHtml::openTag('div', array('class'=>'row form_info cmsms_input'));
				$fieldCaptcha .= CWidget::create('CCaptcha', array('type'=> 'math', 'required'=>true, 'name'=>'captchaWebformResult', 'return' => true));
				if($required) $fieldCaptcha .= CHtml::tag('p', array('class'=>'error', 'style'=>'display:none', 'id'=>'captchaError'), A::t('webforms', 'The field captcha cannot be empty!'));
			$fieldCaptcha .= CHtml::closeTag('div').self::NL;
		}

		$output .= $fieldName;
		$output .= $fieldEmail;
		$output .= $fieldPhone;
		$output .= $fieldCompany;
		$output .= $fieldMessage;
		$output .= $fieldCaptcha;

		$output .= CHtml::tag('div', array('class'=>'cl'), '').self::NL;
		$output .= CHtml::openTag('div', array('class'=>'fl'));
			$output .= CHtml::tag('button', array('type'=>'button', 'class' => 'button_small', 'data-sending'=>A::t('webforms', 'Sending...'), 'data-send'=>A::t('webforms', 'Send'), 'onclick'=>'javascript:webforms_SubmitForm(this)'), A::t('webforms', 'Send')).self::NL;
		$output .= CHtml::closeTag('div').self::NL; /* fl */

		$output .= CHtml::tag('div', array('class'=>'spacer'), '').self::NL;
		$output .= CHtml::closeTag('div').self::NL; /* contact-form-content */
		$output .= CHtml::closeForm().self::NL;
		$output .= CHtml::closeTag('div').self::NL; /* cmsms-form-builder */
		$output .= CHtml::closeTag('div').self::NL; /* cmsms_cc */
		$output .= CHtml::closeTag('div').self::NL; /* entry */
		// Register module javascript css
		A::app()->getClientScript()->registerScriptFile('assets/modules/webforms/js/webforms.js');
		A::app()->getClientScript()->registerCssFile('assets/modules/webforms/css/webforms.css');

		return $output;
	}

}