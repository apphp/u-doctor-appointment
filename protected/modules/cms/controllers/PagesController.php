	...
	public function viewAction($id = 0)
	{
		...
		if(is_array($modulesCodes)){
			...
			foreach($modulesCodes as $key => $val){
				...
				if(preg_match('/'.$val['value'].'/i', $moduleText) || preg_match('/{'.trim($val['value'], '{}').'\|(.*?)}/i', $moduleText, $matches)){
					$componentName = A::app()->mapAppModuleClass((!empty($val['class_code']) ? $val['class_code'] : $key).'Component');
					if(empty($componentName)){
						$componentName = ucfirst($key.'Component');
					}

					if(method_exists($componentName, 'drawShortcode')){
						$pattern = !empty($matches) ? str_replace('|', '\\|', $matches[0]) : $val['value'];
						$pattern = '/'.$pattern.'/i';
						if(!empty($matches[1])){
							// Prepare params
