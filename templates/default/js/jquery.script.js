...
jQuery(document).ready(function() { 
	...
	(function ($) { 
		...
		$('.cmsms_social').socicons( { 
			...
			imagesurl : 'templates/default/images/share_icons/'
		} );
	} )(jQuery);
		...
		if ($(window).width() >= 1024) {
			$('.search_but').bind('click', function () { 
				if ($(this).parents('.wrap_header_search').hasClass('open')) {
					...
					$(this).parents('.wrap_header_search').find('input[name="keywords"]').focus();
				}
	...
	(function ($) { 
		...
		$('.tab ul.tabs li a').click(function (g) {
			var tab = $(this).closest('.tab'), 
				index = $(this).closest('li').index();
            var typeTab = $(this).data('typeTab');

            if(typeTab == "deactivate"){
            	return false;
            }
	...
	(function ($) { 
		...
		$('.tour_content ul.tour li a').click(function (f) {
			...
			if(type == 'all'){
				tour.find('div.tour_box_content').find('div.tour_box').slideDown();
			}else{
