...
class CDataForm extends CWidgs
{
     ...
     *       'messagesSource'=>'core',
	 * 		 'messages'=>array('insert'=>array('success'=>'', 'error'=>''), 'update'=>array('success'=>'', 'error'=>''));
     *       'showAllErrors'=>false,
    ...
    public static function init($params = array())
    {
		...
		$messages 				= self::params('messages', array());
			...
			if($result['error']){
				...
				if(!$msgType){
					...
					if($records->save()){
						...
						// Prepare messages
						if($operationType == 'add'){
							$msg = !empty($messages['insert']['success']) ? $messages['insert']['success'] : 'The adding operation has been successfully completed!';
						}else{
							$msg = !empty($messages['update']['success']) ? $messages['update']['success'] : 'The updating operation has been successfully completed!';
						}
						$msgType = 'success';
						...
						if(APPHP_MODE == 'demo'){
							...
							// Prepare messages
							if($operationType == 'add'){
								$msg = !empty($messages['insert']['success']) ? $messages['insert']['success'] : 'The error occurred while adding new record! To get more information please turn on debug mode.';
							}else{
								$msg = !empty($messages['update']['success']) ? $messages['update']['success'] : 'The error occurred while updating this record! To get more information please turn on debug mode.';
							}
                            $msgType = 'error';
                        }
					}
